...
const bitset<1> Inter_check_SingletonD::predefined(string("1"));

Inter_check_SingletonD::Inter_check_SingletonD(const ConnT_check_SingletonD &connector) : Interaction(connector) {
    involvedPorts.reset();
...
Inter_check_SingletonD::Inter_check_SingletonD(const ConnT_check_SingletonD &connector, bool p) : Interaction(connector) {
    involvedPorts.reset();
...
Inter_check_SingletonD::Inter_check_SingletonD(const ConnT_check_SingletonD &connector, const vector<Port *> &ports) : Interaction(connector), port_vector_fresh(false) {
    commonRecycle(ports);
    mPorts.reserve(1);
}

Inter_check_SingletonD::~Inter_check_SingletonD(){
}

// This one is 'const' but its only role
// is to set mPorts correctly (ie. its only role is to modify the object)
void Inter_check_SingletonD::refreshPorts() const {
...
vector<Port*>& Inter_check_SingletonD::ports() {
...
const vector<Port *> & Inter_check_SingletonD::ports() const {
    if (!port_vector_fresh){
        refreshPorts();
    }
...
void Inter_check_SingletonD::recycle() {
    Interaction::recycle();
    mPorts.clear();
    port_vector_fresh = true;
    defined = false;
...
void Inter_check_SingletonD::recycle(const vector<Port *> &ports) {
    Interaction::recycle();
    commonRecycle(ports);
}

void Inter_check_SingletonD::commonRecycle(const vector<Port *> &ports){
    involvedPorts.reset();
